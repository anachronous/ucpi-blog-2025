...
However, both of these issues can be fixed. To fix content input, one could add an input device such as a braille keyboard or something similar. Maybe even a device dedicated for shortcuts such as an Elgato Stream Deck? On the other hand, other physical modifications would need to be made to fix the size adaption. An idea i had was that you could make the board modular somehow and have the user be able to stick together small "bricks" of boards that stick together like legos. The last big issue of magnet strength can be fixed by introducing electromagnets which could be controlled over software or a physical knob.
